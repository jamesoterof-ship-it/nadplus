...
(function () {
  ...
  var esc = function (s) { return String(s == null ? '' : s).replace(/[&<>"]/g, function (m) {
    return { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[m]; }); };
  function _conPago(n) { return window.PAGO_MODO === 'card' ? Math.floor(n * 0.93 / 100) * 100 : n; }
  ...
  function pintarPrecio() {
    ...
    if ($('sumTot')) $('sumTot').textContent = pesos(_conPago(k.precio));
    var _rot = document.querySelector('.summary .tot span');
    if (_rot) _rot.textContent = (window.PAGO_MODO === 'card' ? 'Total pagando con tarjeta' : 'Total a pagar al recibir');
    var _bt = document.querySelector('#fPedido button[type="submit"]');
    if (_bt && !_bt.disabled) _bt.textContent = (window.PAGO_MODO === 'card' ? 'Pagar ahora con tarjeta' : 'Comprar - pago al recibir');
  }
  ...
  /* enviar el pedido */
  window.PAGO_MODO = window.PAGO_MODO || 'cod';
  /* pago con tarjeta: se atiende aqui y no sigue al registro normal */
  $('fPedido').addEventListener('submit', function (ev) {
    if (window.PAGO_MODO !== 'card') return;
    ev.preventDefault(); ev.stopImmediatePropagation();
    var g = function (x) { return ($(x) && $(x).value || '').trim(); };
    var k = p.packs[elegido];
    var NL = String.fromCharCode(10);
    var txt = 'Hola, quiero PAGAR CON TARJETA por adelantado (7% de descuento).' + NL + NL
      + 'Producto: ' + p.nombre + ' - ' + k.texto + NL
      + 'Total con descuento: ' + pesos(_conPago(k.precio))
      + (g('fNombre') ? NL + 'Nombre: ' + g('fNombre') : '')
      + (g('fDir') ? NL + 'Direccion: ' + g('fDir') : '')
      + (g('fComuna') ? NL + 'Comuna: ' + g('fComuna') : '');
    window.open('https://wa.me/56964775539?text=' + encodeURIComponent(txt), '_blank', 'noopener');
  }, true);
  (function () {
    var caja = $('pagoSel'); if (!caja) return;
    caja.addEventListener('click', function (e) {
      var b = e.target.closest('.po'); if (!b) return;
      window.PAGO_MODO = b.getAttribute('data-p');
      [].forEach.call(caja.querySelectorAll('.po'), function (x) { x.classList.toggle('on', x === b); });
      pintarPrecio();
    });
  })();
  $('fPedido').addEventListener('submit', function (ev) {
